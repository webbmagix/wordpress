...
function wmgx_key_validation_form() {
	...
	if($_POST["key1"])
	{
		...
		$result .= "Keys ". $key1 . $key2 ."!";
		//echo "Keys ". $key1 . $key2 ."!"; // Success Message
	}
    echo $result;
}

add_shortcode('wmgx2_key_validator', 'wmgx_key_validation');
...
function wmgx_html_display_form()
{
  ...
  <form action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" method="post" id="wmgx_key_validation_form">
    ...
    <p><input type="button" name="wmgx_key_validation" value="Validate Keys" id="wmgx_key_validation_btn"></p>
